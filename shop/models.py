...
class Book(models.Model):
    ...
    authors = models.ManyToManyField(
        Author, related_name="books", verbose_name="Auteurs"
    )

    # Description et contenu
    description = RichTextField(verbose_name="Description")
    short_description = models.TextField(
        max_length=500, blank=True, verbose_name="Description courte"
    )
    ...
    format = models.CharField(
        max_length=20, choices=format_choices, default="broche", verbose_name="Format"
    )

    # Images
    cover_image = models.ImageField(
        upload_to=book_cover_upload_path, verbose_name="Image de couverture"
    )
    ...
    preorder_available_date = models.DateField(
        null=True, blank=True, verbose_name="Date de disponibilité prévue"
    )
    preorder_max_quantity = models.PositiveIntegerField(
        null=True,
        blank=True,
        verbose_name="Quantité maximale de précommandes (None = illimité)",
    )
    preorder_current_quantity = models.PositiveIntegerField(
        default=0, verbose_name="Nombre de précommandes effectuées"
    )
    preorder_original_date = models.DateField(
        null=True, blank=True, verbose_name="Date originale de précommande"
    )
    ...
    meta_description = models.TextField(
        max_length=300, blank=True, verbose_name="Description SEO"
    )
    ...
    def is_available_for_preorder(self):
        ...
        if (
            self.preorder_available_date
            and self.preorder_available_date <= timezone.now().date()
        ):
            return False
    ...
    def can_preorder(self, quantity=1):
        ...
        if self.preorder_max_quantity is not None:
            return (
                self.preorder_current_quantity + quantity
            ) <= self.preorder_max_quantity
        return True
...
class BookImage(models.Model):
    ...
    book = models.ForeignKey(
        Book, on_delete=models.CASCADE, related_name="images", verbose_name="Livre"
    )
    image = models.ImageField(upload_to=book_gallery_upload_path, verbose_name="Image")
...
class Review(models.Model):
    ...
    book = models.ForeignKey(
        Book, on_delete=models.CASCADE, related_name="reviews", verbose_name="Livre"
    )
    user = models.ForeignKey(
        "accounts.User", on_delete=models.CASCADE, verbose_name="Utilisateur"
    )
    rating = models.PositiveIntegerField(
        choices=[(i, i) for i in range(1, 6)], verbose_name="Note"
    )
...
class Cart(models.Model):
    ...
    session_key = models.CharField(
        max_length=40, blank=True, null=True, verbose_name="Clé de session"
    )
    session_data = models.JSONField(
        default=empty_dict, blank=True, verbose_name="Données de session"
    )
    created_at = models.DateTimeField(
        auto_now_add=True, verbose_name="Date de création"
    )
...
class CartItem(models.Model):
    ...
    cart = models.ForeignKey(
        Cart, on_delete=models.CASCADE, related_name="items", verbose_name="Panier"
    )
...
class Order(models.Model):
    ...
    billing_first_name = models.CharField(
        max_length=50, blank=True, verbose_name="Prénom facturation"
    )
    billing_last_name = models.CharField(
        max_length=50, blank=True, verbose_name="Nom facturation"
    )
    billing_address = models.TextField(blank=True, verbose_name="Adresse facturation")
    billing_city = models.CharField(
        max_length=100, blank=True, verbose_name="Ville facturation"
    )
    billing_postal_code = models.CharField(
        max_length=20, blank=True, verbose_name="Code postal facturation"
    )
    billing_country = models.CharField(
        max_length=100, default="France", verbose_name="Pays facturation"
    )
    ...
    shipping_cost = models.DecimalField(
        max_digits=8, decimal_places=2, default=0, verbose_name="Frais de port"
    )
    tax_amount = models.DecimalField(
        max_digits=8, decimal_places=2, default=0, verbose_name="Montant des taxes"
    )
    ...
    estimated_delivery = models.DateField(
        null=True, blank=True, verbose_name="Date de livraison estimée"
    )
    actual_delivery = models.DateField(
        null=True, blank=True, verbose_name="Date de livraison effective"
    )
    ...
    admin_viewed_at = models.DateTimeField(
        null=True, blank=True, verbose_name="Date de consultation par l'admin"
    )
    ...
    preorder_ready_date = models.DateField(
        null=True,
        blank=True,
        verbose_name="Date où la précommande est devenue disponible",
    )
    preorder_original_date = models.DateField(
        null=True, blank=True, verbose_name="Date initiale promise pour la précommande"
    )
    preorder_date_changed = models.BooleanField(
        default=False, verbose_name="Date de précommande modifiée"
    )
    preorder_date_change_notified = models.BooleanField(
    ...
    def can_be_cancelled(self):
        ...
        if self.is_preorder and self.status not in [
            "shipped",
            "delivered",
            "cancelled",
            "refunded",
    ...
    def get_status_display_with_date(self):
        ...
        if self.status == "processing" and self.processing_date:
            ...
            return (
                f"{status_display} (depuis le {self.shipped_date.strftime('%d/%m/%Y')})"
            )
...
class OrderItem(models.Model):
    ...
    unit_price = models.DecimalField(
        max_digits=8, decimal_places=2, verbose_name="Prix unitaire"
    )
    total_price = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name="Prix total"
    )
...
class Payment(models.Model):
    ...
    order = models.OneToOneField(
        Order, on_delete=models.CASCADE, related_name="payment", verbose_name="Commande"
    )
    ...
    paypal_payment_id = models.CharField(
        max_length=100, blank=True, verbose_name="ID de paiement PayPal"
    )
    paypal_payer_id = models.CharField(
        max_length=100, blank=True, verbose_name="ID du payeur PayPal"
    )
    paypal_token = models.CharField(
        max_length=100, blank=True, verbose_name="Token PayPal"
    )
...
class Refund(models.Model):
    ...
    amount = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name="Montant du remboursement"
    )
    ...
    paypal_refund_id = models.CharField(
        max_length=100, blank=True, verbose_name="ID remboursement PayPal"
    )
    paypal_status = models.CharField(
        max_length=50, blank=True, verbose_name="Statut PayPal"
    )

    # Métadonnées
    requested_by = models.ForeignKey(
        "accounts.User", on_delete=models.CASCADE, verbose_name="Demandé par"
    )
...
class LoyaltyProgram(models.Model):
    ...
    discount_value = models.DecimalField(
        max_digits=8, decimal_places=2, verbose_name="Valeur de la réduction"
    )
    ...
    valid_from = models.DateTimeField(
        default=timezone.now, verbose_name="Valide à partir de"
    )
...
class PromoCode(models.Model):
    ...
    max_uses = models.PositiveIntegerField(
        null=True, blank=True, verbose_name="Nombre maximum d'utilisations"
    )
...
class PromoCodeUse(models.Model):
    ...
    discount_amount = models.DecimalField(
        max_digits=8, decimal_places=2, verbose_name="Montant de la réduction"
    )
...
class UserLoyaltyStatus(models.Model):
    ...
    last_purchase_date = models.DateTimeField(
        null=True, blank=True, verbose_name="Date du dernier achat"
    )
    ...
    def get_available_loyalty_discount(self):
        ...
        real_spent = confirmed_orders.aggregate(total=models.Sum("total_amount"))[
            "total"
        ] or Decimal("0.00")
        ...
        loyalty_program = (
            ...
            .order_by("-min_purchases", "-min_amount")
            .first()
        )
    ...
    def get_real_statistics(self):
        ...
        stats = {
            ...
            "total_spent": confirmed_orders.aggregate(total=models.Sum("total_amount"))[
                "total"
            ]
            or Decimal("0.00"),
            "last_purchase_date": confirmed_orders.order_by("-created_at")
            .first()
            .created_at
...
class Invoice(models.Model):
    ...
    order = models.OneToOneField(
        Order, on_delete=models.CASCADE, related_name="invoice", verbose_name="Commande"
    )
    ...
    billing_city = models.CharField(max_length=100, verbose_name="Ville")
    billing_postal_code = models.CharField(max_length=20, verbose_name="Code postal")
    billing_country = models.CharField(max_length=100, verbose_name="Pays")
    ...
    shipping_cost = models.DecimalField(
        max_digits=10, decimal_places=2, default=0.00, verbose_name="Frais de port"
    )
    total_amount = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name="Montant total"
    )
    ...
    def save(self, *args, **kwargs):
        if not self.invoice_number:
            ...
            last_invoice = (
                Invoice.objects.filter(invoice_number__startswith=f"FAC{year}")
...
class ShopSettings(models.Model):
    ...
    tax_rate = models.DecimalField(
        max_digits=5, decimal_places=2, default=5.5, verbose_name="Taux de TVA (%)"
    )

    # Paramètres généraux
    shop_name = models.CharField(
        max_length=200, default="Éditions Sen", verbose_name="Nom de la boutique"
    )
    shop_email = models.EmailField(
        default="editions.sen@gmail.com", verbose_name="Email de la boutique"
    )
    shop_phone = models.CharField(
        max_length=20, blank=True, verbose_name="Téléphone de la boutique"
    )
...
class WebhookEvent(models.Model):
    """Stockage des événements webhook reçus de PayPal pour audit et debug"""
