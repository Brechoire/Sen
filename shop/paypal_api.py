...
def create_paypal_order(request):
    ...
    try:
        ...
        try:
            ...
            return JsonResponse(
                {"error": "Commande non trouvée ou non autorisée"}, status=404
            )
        ...
        if not access_token:
            return JsonResponse(
                {"error": "Impossible de récupérer le token PayPal"}, status=500
            )
        ...
        if "links" in order_data:
            approval_link = next(
                (link for link in order_data["links"] if link["rel"] == "approve"), None
            )
            if approval_link:
        ...
        return JsonResponse(
            {"error": "Erreur lors de la création de la commande PayPal"}, status=500
        )
...
def capture_paypal_order(request):
    ...
    try:
        ...
        logger.error(f"Erreur API PayPal: {e}")
        return JsonResponse(
            {"error": "Erreur lors de la capture du paiement"}, status=500
        )
    except Exception as e:
        logger.error(f"Erreur inattendue: {e}")
        return JsonResponse({"error": "Erreur interne du serveur"}, status=500)
...
def capture_paypal_order_by_token(paypal_order_id):
    ...
    try:
        ...
        if settings.PAYPAL_MODE == "sandbox":
            ...
            capture_url = (
                f"https://api.paypal.com/v2/checkout/orders/{paypal_order_id}/capture"
            )
...
def process_paypal_refund(refund):
    ...
    try:
        ...
        if settings.PAYPAL_MODE == "sandbox":
            ...
            refund_url = (
                "https://api.paypal.com/v2/payments/captures/{capture_id}/refund"
            )
        ...
        payload = {
            "amount": {"value": str(refund.amount), "currency_code": "EUR"},
            "note_to_payer": f"Remboursement pour la commande {refund.order.order_number}",
        }

        response = requests.post(
            refund_url.format(capture_id=capture_id), json=payload, headers=headers
        )
        ...
        if response.status_code == 201:
            ...
            error_msg = error_data.get("details", [{}])[0].get(
