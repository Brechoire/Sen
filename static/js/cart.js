            ...
            if (cartItems) {
                cartItems.innerHTML = this.cartData.items.map(item => `
                    <div class="flex items-start mb-3 p-2 hover:bg-gray-50 rounded">
                        <a href="/boutique/livre/${item.slug || item.id}/" class="w-12 mr-3 flex-shrink-0 hover:opacity-80 transition-opacity">
                            <div class="aspect-[2/3] bg-gray-100 rounded overflow-hidden">
                                ${item.cover_url ? 
                                    `<img src="${item.cover_url}" alt="${item.title}" class="w-full h-full object-contain">` :
